...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (user) {
      router.push(needsOnboarding === true ? "/onboarding" : "/");
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-6">
      ...
      <div className="w-full max-w-sm bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white border border-gray-200 text-gray-700 font-medium px-4 py-3 rounded-lg hover:bg-gray-50 active:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loginLoading ? (
            <span>リダイレクト中...</span>
          ) : (
